  ...
  var JSP = {
    ...
    reorgBlock: function (Q) {
      ...
      var rows = [
        ...
        g(o.g_rosokon, '统一劳组恳', o.rosokon, this.REORG_ROSOKON_MAX + ' 万人以下'),
    ...
    elecTable: function (Q) {
      ...
      for (i = 0; i < rows.length; i += 1) {
        ...
        hs = r.hist ? ('<span style="opacity:.5">史实 ' + r.hist + '</span>') : '';
        out.push(r.year + '年　众院 <b>' + r.shakai + '</b> / ' + r.total
          + (ds ? '　' + ds : '') + '　参院 ' + r.hc
          + (r.sp ? '　<span style="opacity:.6">分出去的党 ' + r.sp + '</span>' : '')
          + '　' + hs);
      }
      return out.join('<br>');
    },
